...
Let’s say you allow users to switch between different items (e.g., starships in a UI). Switching causes the component to **suspend** and display a fallback UI.
...
function handleShipChange(newShip: string) {
	startTransition(() => {
...
Sometimes, loading is so fast (\~50ms) that a spinner appears for a blink, which is visually jarring. Instead, you can use [`spin-delay`](https://npm.im/spin-delay) to:
...
## 🛠 Use Case: Ship Viewer with Cache + Transitions

Imagine a UI where users can select starships and view their details. We want:

* 🚫 No re-fetching the same ship
* 🔄 Smooth transitions between ship details
* ⚡ No flickering spinner on fast networks

You can achieve this by combining:

* A **cache** using `Map<string, Promise<Ship>>`
...
function getShip(name: string) {
	let promise = shipCache.get(name)
	if (!promise) {
		promise = fetch(`/api/ship/${name}`).then(res => res.json())
...
function ShipComponent() {
	const [selected, setSelected] = useState('falcon')
	const [isPending, startTransition] = useTransition()
	const ship = use(getShip(selected))
	const showSpinner = useSpinDelay(isPending, { delay: 300, minDuration: 350 })

	return (
		<>
			<button onClick={() => startTransition(() => setSelected('falcon'))}>Millennium Falcon</button>
			<button onClick={() => startTransition(() => setSelected('x-wing'))}>X-Wing</button>
			{showSpinner ? (
				<Spinner />
			) : (
				<div style={{ opacity: isPending ? 0.6 : 1 }}>
